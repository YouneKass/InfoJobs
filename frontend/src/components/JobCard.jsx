...
function JobCardFavoriteButton ({ jobId }) {
    ...
    return (
        <button disabled={!isLoggedIn} onClick={() => toggleFavorite(jobId)} aria-label={isFavorite(jobId) ? 'Remove from favorites' : 'Add to favorites'}>
            {isFavorite(jobId) ? '🧑‍💻' : '🧑‍💼'}
        </button>
    )
}
...
function JobCardApplyButton({ jobId }) {
    ...
    const handleApplyClick = () => {
        console.log('Aplicando al trabjo con id:', jobId)
        setIsApplied(true)
    }
